...
/// Object that clusters all arguments and parameters retrieved or
/// produced during a route search.
class ModularArguments {
  /// It retrieves parameters after consulting a dynamic route. If it
  /// is not a dynamic route the object will be an empty Map.
  ...
  /// Retrieved from a direct input of arguments from the
  ...
  /// The value is the empty string if there is no
  ...
  /// Keys in the query string that have no value are mapped
  /// to the empty string. If a key occurs more than once in
  /// the query string, it is mapped to an arbitrary choice of
  /// possible value.
  ...
  /// specified for FORM post in the HTML 4.01 specification section 17.13.4.
  /// Each key and value in the resulting map has been decoded.
  /// If there is no query the map is empty.
  /// Keys are mapped to lists of their values. If a key occurs
  /// only once, its value is a singleton list. If a key occurs
  /// with no value, the empty string is used as the value for that occurrence.
